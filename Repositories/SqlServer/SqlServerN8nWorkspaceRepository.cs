...
public sealed class SqlServerN8nWorkspaceRepository : IN8nWorkspaceRepository
{
    ...
    public async Task<N8nWorkspaceCreadoDto> CrearAsync(long usuarioId, string? ipOrigen, CancellationToken ct = default)
    {
        await using var conn = await _factory.AbrirAsync(ct);
        await using var cmd = new SqlCommand("dbo.sp_CrearWorkspaceN8N", conn)
        {
        ...
        try
        {
            ...
            if (!await reader.ReadAsync(ct))
                throw new InvalidOperationException("sp_CrearWorkspaceN8N no devolvió filas.");
